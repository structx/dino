...
import (
	...
	"github.com/structx/dino/internal/routes/queries"
...
// Active
func (s *serviceImpl) Active(ctx context.Context, hostname string) (Route, error) {
	timeout, cancel := context.WithTimeout(ctx, time.Second*3)
	defer cancel()

	sqlRoute, err := queries.New(s.db).SelectActiveRoute(timeout, hostname)
	if err != nil {
		return Route{}, fmt.Errorf("failed to execute select active route query: %w", err)
	}
	return dtoRoute(sqlRoute), nil
}
