...
type Mux interface {
	InitSession(tunnelnet.ClientConn, string, string, string) error
...
func (s *sessionMultiplexer) RouteMsg(sessionID string, payload []byte) error {
	...
	if !ok {
		return errors.New("session not action")
	}
...
func (s *sessionMultiplexer) InitSession(conn tunnelnet.ClientConn, sessionID, protocol, addr string) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
